...
class JWTStrategy:
    ...
    async def write_token(self, user: UserSchemaSchema) -> str:
        """Generate a JWT token for a user."""
        expires_delta = timedelta(seconds=self.lifetime_seconds)
        token = create_access_token(
            data={"sub": str(user.id), "email": user.email}, expires_delta=expires_delta
        )
        if not token:
...
async def get_current_user(
    ...
    try:
        ...
        result = await db.execute(select(UserSchemaSchema).where(UserSchemaSchema.id == user_id))
