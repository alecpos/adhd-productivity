...
class ErrorDetails(RootModel[Dict[str, List[str]]]):
    ...
    model_config = {
        "json_schema_extra": {"description": "Field-specific error messages, keyed by field name"}
    }
...
class ErrorResponse(BaseModel):
    ...
    details: Optional[Dict[str, List[str]]] = Field({}, description="Field-specific error details")
...
def create_collection_response(
    ...
    return CollectionResponse(items=items, pagination=pagination)
...
def create_error_response(
    ...
    """
    return ErrorResponse(code=code, message=message, details=details or {}).dict()
...
def forbidden_response(
    ...
    return create_error_response(
        code="FORBIDDEN", message=message, status_code=status.HTTP_403_FORBIDDEN
    )
...
def conflict_response(message: str) -> Dict[str, Any]:
    ...
    return create_error_response(
        code="RESOURCE_CONFLICT", message=message, status_code=status.HTTP_409_CONFLICT
    )
...
def server_error_response(message: str = "An unexpected error occurred") -> Dict[str, Any]:
    ...
    return create_error_response(
        code="INTERNAL_ERROR", message=message, status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
    )
