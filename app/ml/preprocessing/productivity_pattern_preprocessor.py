...
class ProductivityPatternPreprocessor:
    ...
    def prepare_time_block_features(
        self,
        ...
        if 'block_type' in df.columns:
            block_types = self.block_type_encoder.transform(
                df['block_type'].values.reshape(-1, 1)
            )
        ...
        if 'priority' in df.columns:
            priorities = self.priority_encoder.transform(
                df['priority'].values.reshape(-1, 1)
            )
        ...
        if 'actual_start_time' in df.columns and 'actual_end_time' in df.columns:
            df['actual_duration'] = df.apply(
                lambda x: (x['actual_end_time'] - x['actual_start_time']).total_seconds() / 60
            ...
            df['duration_accuracy'] = df.apply(
                lambda x: (x['planned_duration'] / x['actual_duration']
        ...
        numeric_features = [
            'energy_level', 'focus_level', 'mental_health_score',
            'completion_rate', 'effectiveness_score',
            'total_focus_time', 'interruption_count',
            'planned_duration'
        ]
    ...
    def prepare_mental_health_features(
        ...
        # Get relevant numeric features for scaling
        numeric_features = [
            'mood_score', 'stress_level', 'anxiety_level', 'anxiety_score',
            'energy_level', 'focus_level', 'sleep_hours', 'sleep_quality'
        ]
    ...
    def create_hourly_sequences(
        ...
        if not time_blocks_df.empty and not mental_health_df.empty:
            ...
            mental_health_hourly = mental_health_df[mental_health_cols].resample(resample_freq).mean()
    ...
    def get_flexible_block_indices(
        self,
        time_blocks: List[Dict[str, Any]]
    ) -> List[int]:
