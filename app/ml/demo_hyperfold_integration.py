...
from app.ml.hyperfold_attention import TemporalPatternPredictor, integrate_hyperfold_with_calendar
...
def generate_mock_data():
    ...
    def circadian_energy(t, day_index=0):
        ...
        base = 0.7 * np.sin(2 * np.pi * t / 24 - np.pi / 2) + 0.3 * np.sin(
            2 * np.pi * t / 12 - np.pi / 3
        )
...
def analyze_productivity_patterns(task_history_df):
    ...
    hourly_success = (
        task_history_df.groupby("hour")["completed_successfully"]
        .agg(["count", "sum", "mean"])  # type: ignore
        .rename(columns={"mean": "success_rate"})
        .reset_index()
    )  # Add reset_index to make 'hour' a column

    # Calculate success rate by day of week
    daily_success = (
        task_history_df.groupby("day_of_week")["completed_successfully"]
        .agg(["count", "sum", "mean"])  # type: ignore
        .rename(columns={"mean": "success_rate"})
        .reset_index()
    )  # Add reset_index to make 'day_of_week' a column

    # Calculate success rate by hour and day of week
    hour_day_success = (
        task_history_df.groupby(["day_of_week", "hour"])["completed_successfully"]
        .agg(["count", "sum", "mean"])  # type: ignore
        .rename(columns={"mean": "success_rate"})
        .reset_index()
    )
...
def train_hyperfold_model(task_history_df, user_energy_curve):
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0, n_samples, batch_size):
            ...
            energy_batch = energy_shuffled[i : i + batch_size].unsqueeze(
                1
            )  # Add sequence dimension

            # Forward pass
            optimizer.zero_grad()
            output = model(features=X_batch, timestamps=ts_batch, energy_levels=energy_batch)
...
def get_optimal_schedule_with_hyperfold(task_features_df, user_energy_curve, model=None):
    ...
    # Make predictions
    with torch.no_grad():
        predictions = model(features=X_tensor, timestamps=ts_tensor, energy_levels=energy_tensor)
    ...
    for _, slot in sorted_slots.iterrows():
        ...
        for _, task in remaining_tasks.iterrows():
            ...
            if energy >= 7:  # High energy
                energy_match = max(
                    0, task["focus_required"] * 0.7 + task["executive_function_load"] * 0.3 - 3
                )
            elif energy >= 5:  # Medium energy
                energy_match = max(
                    0, 5 - abs(task["focus_required"] - 5) - abs(task["creative_required"] - 5)
                )
            else:  # Low energy
                energy_match = max(
                    0, 10 - task["focus_required"] - task["executive_function_load"] * 0.5
                )
...
def visualize_hyperfold_results(
    task_features_df, user_energy_curve, schedule_result, analysis_result
):
    """
    ...
    for i, task in enumerate(scheduled_tasks):
        ...
        if "duration_minutes" in task:
            ...
            if isinstance(task["end_time"], (int, float)) and isinstance(
                task["start_time"], (int, float)
            ):
                task_duration = (task["end_time"] - task["start_time"]) / 3600
            elif hasattr(task["end_time"], "total_seconds") and hasattr(
                ...
                try:
                    if isinstance(task["end_time"], (int, float)) and isinstance(
                        ...
                        end_ts = (
                            ...
                            if hasattr(task["end_time"], "timestamp")
                            else task["end_time"]
                        )
                        start_ts = (
        ...
        ax2.barh(i, task_duration, left=task_start, height=0.8, color=plt.cm.Blues(color_intensity))

        # Add a text label inside the bar if it's wide enough
        if task_duration > 1.5:
            ax2.text(task_start + 0.1, i, f"{task['title']}", va="center", fontsize=8)
    ...
    ax3.bar(hourly_success["hour"], hourly_success["success_rate"], color="purple", alpha=0.7)
    ...
    if (
        "day_of_week" not in hour_day_success.columns
        and "day_of_week" in hour_day_success.index.names
    ):
    ...
    if all(col in hour_day_success.columns for col in ["day_of_week", "hour", "success_rate"]):
        ...
        ax4.text(0.5, 0.5, "Insufficient data for heatmap", ha="center", va="center", fontsize=12)
...
def main():
    ...
    for window in analysis_result["optimal_windows"][:5]:
        print(
            f"  • {window['day_name']} at {window['hour']:02d}:00 - "
    ...
    for task in schedule_result["scheduled_tasks"][:5]:
        ...
        print(
            f"  • {task['title']} - {start_str} to {end_str} "
            f"(Energy: {task['energy_level']:.1f}, Score: {task['optimality_score']:.2f})"
        )

    # Visualize results
    print("\nVisualizing results...")
    visualize_hyperfold_results(
        task_features_df, user_energy_curve, schedule_result, analysis_result
    )
