...
class ModelTrainer:
    ...
    def train_schedule_optimizer(
        ...
        Args:
            time_blocks: List of time block dictionaries containing task information
            energy_patterns: List of energy pattern dictionaries
            work_hours: List of work hour dictionaries
        ...
        X_time = np.array([block["features"] for block in time_blocks])
        X_energy = np.array([pattern["features"] for pattern in energy_patterns])
        X_hours = np.array([hours["features"] for hours in work_hours])
