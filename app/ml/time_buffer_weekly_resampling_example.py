...
class TimeBufferCalculatorDemo:
    ...
    async def weekly_resampling(
        ...
        if (
            "actual_minutes_mean" in weekly_df.columns
            and "week_start" in weekly_df.columns
            ...
            try:
                ...
                if pd.notnull(min_idx) and pd.notnull(max_idx):
                    result["weekly_stats"]["most_efficient_week"] = (
                        weekly_df.loc[min_idx, "week_start"].strftime("%Y-%m-%d")
                        if isinstance(weekly_df.loc[min_idx, "week_start"], datetime)
                        else str(weekly_df.loc[min_idx, "week_start"])
                    )
                    result["weekly_stats"]["least_efficient_week"] = (
                        weekly_df.loc[max_idx, "week_start"].strftime("%Y-%m-%d")
                        if isinstance(weekly_df.loc[max_idx, "week_start"], datetime)
                        else str(weekly_df.loc[max_idx, "week_start"])
                    )
...
def plot_weekly_patterns(result):
    ...
    try:
        ...
        if "week_start" in weekly_transitions.columns:
            ...
            logger.error(
                "Cannot generate plots: No date column (week_start or timestamp) found in data"
            )
            return
        ...
        if (
            "actual_minutes_mean" in weekly_transitions.columns
            and "predicted_minutes_mean" in weekly_transitions.columns
        ):
            plt.subplot(2, 2, 1)
            plt.plot(
                weekly_transitions[date_column],
                weekly_transitions["actual_minutes_mean"],
                "b-",
                label="Actual",
            )
            plt.plot(
                weekly_transitions[date_column],
                weekly_transitions["predicted_minutes_mean"],
                "g--",
                label="Predicted",
            )
            plt.xlabel("Week")
        ...
        if "prediction_error_mean" in weekly_transitions.columns:
            plt.subplot(2, 2, 2)
            plt.plot(
                weekly_transitions[date_column], weekly_transitions["prediction_error_mean"], "r-"
            )
            plt.xlabel("Week")
        ...
        # Plot weekly trend
        if (
            result["patterns"]["weekly_trend"]
            and "actual_minutes_mean" in result["patterns"]["weekly_trend"]
        ):
...
async def main():
    ...
    print(
        f"Avg. transitions per week: {result['weekly_stats']['average_transitions_per_week']:.2f}"
    )
