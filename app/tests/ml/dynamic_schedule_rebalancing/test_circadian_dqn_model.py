...
class TestCircadianDQNModel:
    ...
    def test_init(self):
        ...
        with patch(
            "app.ml.models.energy_optimizer_model.CircadianRhythmModel", MagicMock()
        ) as mock_crm:
    ...
    def test_build_model(self):
        ...
        with patch("app.ml.models.energy_optimizer_model.CircadianRhythmModel", MagicMock()):
            ...
            task_shape = getattr(
                tf_model.inputs[0].shape, "as_list", lambda: tf_model.inputs[0].shape
            )()
            circ_shape = getattr(
                tf_model.inputs[1].shape, "as_list", lambda: tf_model.inputs[1].shape
            )()
            ...
            output_shape = getattr(
                tf_model.outputs[0].shape, "as_list", lambda: tf_model.outputs[0].shape
            )()
    ...
    def test_create_state_with_circadian_features(self):
        ...
        with patch("app.ml.models.energy_optimizer_model.CircadianRhythmModel") as mock_crm_class:
            ...
            state = model.create_state_with_circadian_features(
                task_state=task_state, timestamp=timestamp, user_data=user_data
            )
    ...
    def test_save_load(self):
        ...
        with patch("app.ml.models.energy_optimizer_model.CircadianRhythmModel") as mock_crm_class:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                model1 = CircadianDQNModel(state_size=16, action_size=3, circadian_importance=0.5)
                model1.trained = True
                ...
                with (
                    patch(
                        "app.ml.models.adhd17_reinforcement_model.models.load_model"
                    ) as mock_load_model,
...
class TestTaskCognitiveProfile:
    ...
    def test_get_energy_requirements(self):
        ...
        focus_energy = TaskCognitiveProfile.get_energy_requirements(
            TaskCognitiveProfile.FOCUS_INTENSIVE
        )
        creative_energy = TaskCognitiveProfile.get_energy_requirements(
            TaskCognitiveProfile.CREATIVE
        )
        routine_energy = TaskCognitiveProfile.get_energy_requirements(TaskCognitiveProfile.ROUTINE)
        admin_energy = TaskCognitiveProfile.get_energy_requirements(
            TaskCognitiveProfile.ADMINISTRATIVE
        )
