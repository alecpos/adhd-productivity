...
from app.ml.models.adhd17_reinforcement_model import (
    DQNScheduler,
    CircadianDQNModel,
...
def sample_user_data():
    ...
    return {
        ...
        "sleep_quality": 0.8,
        "sleep_duration": 8.0,
        "medications": [{"name": "Medication A", "time_taken": time(8, 0)}],
        "circadian_profile": {
...
def assert_objects_equal():
    ...
    def _assert(obj1, obj2, fields):
        for field in fields:
            assert getattr(obj1, field) == getattr(
                obj2, field
            ), f"Field '{field}' differs between objects"
