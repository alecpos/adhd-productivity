...
class AdaptiveGamificationEngine:
    ...
    async def track_effectiveness(
        self, user_id: str, action_name: str, engagement_score: float
    ) -> Dict[str, Any]:
...
router = APIRouter(prefix="/gamification", tags=["Gamification", "Motivation"])
...
async def get_user_motivation_profile(
    ...
    try:
        ...
        engine = init_gamification_engine(
            get_gamification_service(db), get_user_insights_service(db)
        )
        profile = await engine.get_user_profile(str(current_user.id))
        return profile
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve motivation profile: {str(e)}"
        )
...
async def update_user_motivation_profile(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update motivation profile: {str(e)}"
        )
...
async def get_gamification_actions(
    ...
    try:
        ...
        actions = await engine.get_gamification_actions(str(current_user.id), context, count)
...
async def get_optimal_mechanics(
    ...
    try:
        ...
        mechanics = await engine.get_optimal_mechanics(str(current_user.id), context, limit)
        return mechanics
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get optimal game mechanics: {str(e)}"
        )
...
async def track_gamification_effectiveness(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to track gamification effectiveness: {str(e)}"
        )
