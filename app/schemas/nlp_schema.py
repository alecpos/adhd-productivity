...
class NLPParserRequestSchema(BaseSchema):
    ...
    user_preferences: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="User-specific parsing preferences"
    )
...
class NLPParserResponseSchema(BaseSchema):
    ...
    entities: List[Dict[str, Any]] = Field(
        default_factory=list, description="Named entities found in text"
    )
...
class NLPTaskParseSchema(BaseSchema):
    ...
    meta_data: Dict[str, Any] = Field(
        default_factory=dict, description="Additional parsed metadata"
    )
...
class TaskComplexityAnalysisSchema(BaseSchema):
    ...
    focus_requirements: Dict[str, float] = Field(
        ..., description="Different types of focus required"
    )
    potential_challenges: List[str] = Field(..., description="ADHD-specific challenges")
    breakdown_suggestions: List[Dict[str, Any]] = Field(
        ..., description="Suggestions for breaking down task"
    )
    energy_level_recommendation: str = Field(
        ..., description="Best energy level to tackle this task"
    )
...
class FocusStrategySchema(BaseSchema):
    ...
    effectiveness_rating: Optional[float] = Field(
        None, ge=0, le=1, description="User-specific effectiveness rating"
    )
...
class NLPProcessingOptionsSchema(BaseSchema):
    ...
    ai_model_settings: Dict[str, Any] = Field(
        default_factory=dict, description="Model-specific settings"
    )
    analysis_depth: str = Field(
        default="standard", description="Depth of analysis: quick, standard, or deep"
    )
    custom_entities: Optional[List[Dict[str, Any]]] = Field(
        None, description="Custom entity definitions"
    )
    confidence_threshold: float = Field(
        default=0.7, ge=0, le=1, description="Minimum confidence score threshold"
    )
