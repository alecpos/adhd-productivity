...
class TimeManagementService:
    ...
    async def schedule_blocks(
        ...
        return ScheduleResponseSchema(blocks=blocks, total_focus_time=total_focus_time, total_breaks=total_breaks)
    ...
    async def end_focus_session(self, user_id: UUID, session_id: UUID) -> FocusSessionSchema:
        ...
        session = await self.db.execute(select(FocusSessionSchema).where(and_(FocusSessionSchema.id == session_id, FocusSessionSchema.user_id == user_id)))
        session = session.scalar_one_or_none()
        if not session:
            raise ValueError('Session not found')
        session.end_time = datetime.utcnow()
        actual_duration = (session.end_time - session.start_time).total_seconds() / 60
        session.metrics = {**(session.metrics or {}), 'actual_duration': actual_duration, 'completion_rate': actual_duration / session.metrics['planned_duration'] * 100}
        await self.db.commit()
        await self.db.refresh(session)

    async def get_time_analytics(self, user_id: UUID, start_date: datetime, end_date: datetime) -> DictSchema:
        """Get analytics about time usage and productivity."""
        blocks_query = await self.db.execute(select(TimeBlockModel).where(and_(TimeBlockModel.user_id == user_id, TimeBlockModel.start_time >= start_date, TimeBlockModel.start_time <= end_date)))
        blocks = blocks_query.scalars().all()
        sessions_query = await self.db.execute(select(FocusSessionSchema).where(and_(FocusSessionSchema.user_id == user_id, FocusSessionSchema.start_time >= start_date, FocusSessionSchema.start_time <= end_date)))
        sessions = sessions_query.scalars().all()
        total_focus_time = sum(((block.end_time - block.start_time).total_seconds() / 3600 for block in blocks if block.block_type != BlockType.BREAK))
        completed_sessions = [s for s in sessions if s.end_time is not None]
        avg_completion_rate = sum((s.metrics.get('completion_rate', 0) for s in completed_sessions)) / len(completed_sessions) if completed_sessions else 0
        return {'total_focus_hours': round(total_focus_time, 2), 'completed_sessions': len(completed_sessions), 'average_completion_rate': round(avg_completion_rate, 2), 'most_productive_hours': self._calculate_productive_hours(blocks)}
