...
class GamificationService(BaseService[StreakModel, StreakResponse, PointsResponse]):
    ...
    @with_concurrency_control(
        ...
        error_message="Failed to update user points due to concurrent modification",
    )
    async def add_user_points(self, user_id: UUID, points_to_add: int) -> Dict[str, Any]:
        """Add points to a user."""
        query = (
            select(PointsModel).where(PointsModel.user_id == user_id).with_for_update(nowait=True)
        )
        result = await self.db.execute(query)
        ...
        if len(points_list) > 1:
            ...
            points = PointsModel(
                ...
                level=self._calculate_level(points_to_add),
                created_at=datetime.utcnow(),
                updated_at=datetime.utcnow(),
            )
            self.db.add(points)
        leaderboard_query = (
            ...
            .where(and_(LeaderboardModel.user_id == user_id, LeaderboardModel.category == "daily"))
        ...
        if not leaderboard_entries:
            leaderboard_entry = LeaderboardModel(
                user_id=user_id,
                category="daily",
                score=float(points.total_points),
                rank=0,
    ...
    async def update_user_streak(self, user_id: UUID) -> Dict[str, Any]:
        ...
        query = select(StreakModel).where(
            and_(StreakModel.user_id == user_id, StreakModel.streak_type == "daily")
        )
        result = await self.db.execute(query)
    ...
    async def _award_streak_badge(self, user_id: UUID, milestone: int) -> Optional[Dict[str, Any]]:
        ...
        badge_names = {
            3: "StreakModel Starter",
            7: "Weekly Warrior",
            14: "Fortnight Fighter",
            30: "Monthly Master",
            60: "StreakModel Sage",
            90: "StreakModel Legend",
        }
        badge_name = badge_names.get(milestone)
        if not badge_name:
            query = select(BadgeModel).where(
                and_(BadgeModel.user_id == user_id, BadgeModel.name == badge_name)
            )
            result = await self.db.execute(query)
    ...
    async def get_leaderboard(self, category: str) -> Dict[str, Any]:
        ...
        query = (
            select(LeaderboardModel)
            .where(LeaderboardModel.category == category)
            .order_by(desc(LeaderboardModel.score))
        )
        result = await self.db.execute(query)
        ...
        return {
            "category": category,
            "leaderboard": [
                {"user_id": str(entry.user_id), "score": entry.score, "rank": entry.rank}
    ...
    async def add_user_to_leaderboard(self, category: str, user_id: UUID) -> Dict[str, Any]:
        ...
        query = (
            select(LeaderboardModel)
            .where(and_(LeaderboardModel.user_id == user_id, LeaderboardModel.category == category))
            .with_for_update(nowait=True)
        )
        try:
            result = await self.db.execute(query)
            ...
            if entries:
                ...
                return {
                    ...
                    "message": "User already in leaderboard",
                }
                ...
                result = await self.db.execute(
                    select(LeaderboardModel).where(
                        and_(
                            ...
                            LeaderboardModel.category == category,
                        )
                    )
                )
                entries = result.scalars().all()
    ...
    async def get_user_dashboard(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            streak_query = select(StreakModel).where(
                and_(StreakModel.user_id == user_id, StreakModel.streak_type == "daily")
            )
            ...
            if streak:
                streak_data = {
                    ...
                    "longest_streak": streak.longest_streak,
                    "last_activity": (
                        streak.last_activity.isoformat() if streak.last_activity else None
                    ),
                }
            points_data = {
                "total_points": points.get("total_points", 0),
                "level": points.get("level", 1),
                "message": points.get("message", "No points data available"),
            }
            return {
                "streaks": streak_data,
                "leaderboard": {
                    "entries": (
                        [
                            {"category": entry.category, "score": entry.score, "rank": entry.rank}
        ...
        return [
            {
                ...
                "meta_data": achievement.meta_data,
            }
            for achievement in achievements
        ]

    async def _award_points_achievement(
        self, user_id: UUID, milestone: int
        ...
        if not achievement_name:
            query = select(AchievementModel).where(
                and_(AchievementModel.user_id == user_id, AchievementModel.name == achievement_name)
            )
            result = await self.db.execute(query)
            ...
            if not existing_achievement:
                achievement = AchievementModel(
                    ...
                    points=milestone // 10,
                    earned_at=datetime.now(timezone.utc),
                    meta_data={"milestone": milestone, "progress": 100, "completed": True},
                )
                ...
                return {
                    "name": achievement.name,
                    "description": achievement.description,
                    "bonus_points": achievement.points,
                }
    ...
    async def _check_point_achievements(
        self, user_id: UUID, total_points: int
    ) -> List[Dict[str, Any]]:
    ...
    async def update_leaderboard_score(
        self, category: str, user_id: UUID, score: float
    ) -> Dict[str, Any]:
        ...
        while retry_count < max_retries:
            try:
                ...
                await self.db.rollback()
                if (
                    isinstance(e, sqlalchemy.exc.DBAPIError)
                    and "could not obtain lock" in str(e)
                    or (
                        isinstance(e, sqlalchemy.exc.OperationalError)
                        and "could not obtain lock" in str(e)
                    )
                ):
