...
class ProductivityService(BaseService):
    ...
    async def end_focus_session(
        ...
        user_id: UUID,
        session_id: UUID,
        interruptions: Optional[List[InterruptionDict]] = None,
    ) -> FocusSessionModel:
        ...
        async with self.error_handler("ending focus session"):
            session = await self.db.execute(
                select(FocusSessionModel).where(
                    and_(FocusSessionModel.id == session_id, FocusSessionModel.user_id == user_id)
                )
            )
            ...
            session.metrics["actual_duration"] = (
    ...
    async def _analyze_focus_sessions(
        self, user_id: UUID, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        ...
        total_duration = sum(
            ((s.end_time - s.start_time).total_seconds() / 3600 for s in completed_sessions)
        )
    ...
    async def _get_active_session(self, user_id: UUID) -> Optional[FocusSessionModel]:
        ...
        result = await self.db.execute(
            select(FocusSessionModel).where(
                and_(FocusSessionModel.user_id == user_id, FocusSessionModel.end_time.is_(None))
            )
        )
    ...
    def _calculate_success_patterns(self, sessions: List[FocusSessionModel]) -> Dict[str, Any]:
        ...
        return {
            ...
            "environment_scores": {
                env: sum(scores) / len(scores) for env, scores in environment_scores.items()
            },
        }

    def _calculate_optimal_times(
        self, energy_patterns: Dict[str, Any], focus_analytics: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
