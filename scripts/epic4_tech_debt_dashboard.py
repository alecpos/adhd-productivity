...
class Epic4TechDebtDashboard:
    ...
    def run_debt_detection(self) -> Dict[str, Any]:
        ...
        try:
            cmd = ['python', os.path.join(SCRIPT_DIR, 'detect_tech_debt.py'), 
                   '--all', '--json', '--epic', 'Epic 4']
    ...
    def _plot_subcategory_distribution(self, results: List[Dict[str, Any]]) -> str:
        ...
        # Sort by count
        labels = [k for k, v in sorted(subcategory_counts.items(), key=lambda x: x[1], reverse=True)]
    ...
    def _plot_severity_by_subcategory(self, results: List[Dict[str, Any]]) -> str:
        ...
        df.plot(kind='bar', stacked=True, ax=ax, 
                color=['#ff0000', '#ff9900', '#ffcc00', '#99cc00'])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Epic 4 technical debt dashboard")
    parser.add_argument(
        "--output-dir", help="Output directory for dashboard")
