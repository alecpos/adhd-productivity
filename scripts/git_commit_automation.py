...
def get_file_timestamps(file_path: str) -> Optional[FileTimestamp]:
    ...
    try:
        ...
        return FileTimestamp(
            path=file_path, mtime=stat.st_mtime, ctime=stat.st_ctime, size=stat.st_size
        )
    except OSError as e:
...
def create_commit_for_date(
    date: str, files: List[FileTimestamp], commit_message: Optional[str] = None
) -> bool:
...
def main():
    ...
    parser.add_argument(
        "--commit-message", help="Custom commit message template (use {date} for date placeholder)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if not tracked_files:
        logger.warning(
            "No files found to commit. Make sure you have files that aren't ignored by .gitignore"
        )
        return
