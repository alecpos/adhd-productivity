...
def create_weekly_trend_chart(data):
    ...
    if 'predicted_minutes_mean' in df.columns:
        plt.plot(df['timestamp'], df['predicted_minutes_mean'], 's--', label='Predicted Minutes (Mean)')
...
def create_day_of_week_chart(data):
    ...
    plt.bar(x, sorted_means, yerr=sorted_stds, align='center', alpha=0.7, ecolor='black', capsize=10)
...
def create_heatmap(data):
    ...
    if len(metric_cols) > 0 and 'week_number' in df.columns:
        ...
        core_metrics = [col for col in metric_cols if col in [
            'actual_minutes_mean', 'predicted_minutes_mean',
            'prediction_error_mean', 'context_score_mean'
        ]]
        ...
        # Plot the heatmap
        sns.heatmap(pivot_df, annot=False, cmap="YlGnBu", linewidths=.5, fmt=".2f")
